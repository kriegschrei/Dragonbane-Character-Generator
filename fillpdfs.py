...
def write_fillable_pdf(input_pdf_path, output_pdf_path, data_dict, flatten=False,force_combo_value=False):
    """
        ...
        if Page[ANNOT_KEY]:
            for annotation in Page[ANNOT_KEY]:
                ...
                if target and annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    key = target[ANNOT_FIELD_KEY].to_unicode() # Remove parentheses
                    target_aux = target
                    while target_aux['/Parent']:
                        key = target['/Parent'][ANNOT_FIELD_KEY].to_unicode() + '.' + key
                    ...
                    if key in data_dict.keys():
                        if target[ANNOT_FORM_type] == ANNOT_FORM_button:
                        ...
                        elif target[ANNOT_FORM_type] == ANNOT_FORM_combo and force_combo_value:
                            # Drop Down combo, force the value
                            ...
                            if type(data_dict[key]) == list:
                                export = []
                                # If no options are found, use the key itself
                                if not options:
                                    options = ['1 H', '2 H', '\x85']
                                for each in options:
                                    if each.to_unicode() in data_dict[key]:
                                ...
                                if not options:
                                    options = [ f'{data_dict[key]}' ]
                                for each in options:
                                    try:
                                        each_stripped = each.to_unicode()
                                    except:
                                        each_stripped = each
                                    #print(each.to_unicode())
                                    if each_stripped == data_dict[key]:
                                        export = each
                                if export is None:
                                    if data_dict[key] != "None" and data_dict[key] != "":
                                        pass
                                        raise KeyError(f"{data_dict[key]} Not An Option For {annotation[ANNOT_FIELD_KEY]}, Options are {options}")
                                pdfstr = pdfrw.objects.pdfstring.PdfString.encode(f'{data_dict[key]}')
                            annotation.update(pdfrw.PdfDict(V=pdfstr, AS=pdfstr))
                        elif target[ANNOT_FORM_type] == ANNOT_FORM_text:
                            # regular text field
                            target.update( pdfrw.PdfDict( V=data_dict[key], AP=data_dict[key]) )
                            if target[ANNOT_FIELD_KIDS_KEY]:
                                target[ANNOT_FIELD_KIDS_KEY][0].update( pdfrw.PdfDict( V=data_dict[key], AP=data_dict[key],DV=data_dict[key]) )
                if flatten == True:
